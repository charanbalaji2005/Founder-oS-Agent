...
import { useManager } from "@/hooks/useResearch";
...
import { ArrowLeft, RefreshCw, AlertTriangle, Calendar, ChevronRight } from "lucide-react-native";

export default function ManagerScreen() {
  const { content, status, error, hasData } = useManager();
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <View style={styles.topBar}>
        ...
        <Text style={styles.topTitle}>Manager Agent</Text>
        {hasData && (
          <TouchableOpacity onPress={() => rerunAgent("manager")} style={styles.regenBtn}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {status === "running" && (
          <View style={styles.loadingBox}>
            <Loader size={32} label="Structuring team sprints, deliverables & roadmaps..." color={COLORS.accent} />
          </View>
        )}
        ...
        {!hasData && status === "idle" && (
          <View style={styles.empty}>
            <Calendar size={48} color={COLORS.textMuted} style={{ marginBottom: 14, opacity: 0.4 }} />
            <Text style={styles.emptyTitle}>No Project Plan yet</Text>
            <Text style={styles.emptySub}>
              Run the startup generator pipeline to get structured timeline plans.
            </Text>
            <Button
              label="Generate Plan"
          ...
          <>
            ...
            <View style={styles.navRow}>
              <Button
                ...
                onPress={() => router.push("/startup/employee")}
                variant="ghost"
                size="sm"
