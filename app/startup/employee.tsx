...
import { useEmployee } from "@/hooks/useResearch";
...
import { ArrowLeft, RefreshCw, AlertTriangle, Wrench, ChevronRight } from "lucide-react-native";

export default function EmployeeScreen() {
  const { content, status, error, hasData } = useEmployee();
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <View style={styles.topBar}>
        ...
        <Text style={styles.topTitle}>Employee Agent</Text>
        {hasData && (
          <TouchableOpacity onPress={() => rerunAgent("employee")} style={styles.regenBtn}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {status === "running" && (
          <View style={styles.loadingBox}>
            <Loader size={32} label="Creating detailed action checklists & daily operations SOPs..." color={COLORS.accent} />
          </View>
        )}
        ...
        {!hasData && status === "idle" && (
          <View style={styles.empty}>
            <Wrench size={48} color={COLORS.textMuted} style={{ marginBottom: 14, opacity: 0.4 }} />
            <Text style={styles.emptyTitle}>No Task SOPs yet</Text>
            <Text style={styles.emptySub}>
              Run the startup generator pipeline to get operational action guides.
            </Text>
            <Button
              label="Generate Plan"
          ...
          <>
            ...
            <View style={styles.navRow}>
              <Button
                ...
                onPress={() => router.push("/startup/customer_service")}
                variant="ghost"
                size="sm"
