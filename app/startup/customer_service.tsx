...
import { useCustomerService } from "@/hooks/useResearch";
...
import { ArrowLeft, RefreshCw, AlertTriangle, Headset, ChevronRight } from "lucide-react-native";

export default function CustomerServiceScreen() {
  const { content, status, error, hasData } = useCustomerService();
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <View style={styles.topBar}>
        ...
        <Text style={styles.topTitle}>Customer Service Agent</Text>
        {hasData && (
          <TouchableOpacity onPress={() => rerunAgent("customer_service")} style={styles.regenBtn}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {status === "running" && (
          <View style={styles.loadingBox}>
            <Loader size={32} label="Designing onboarding help loops, FAQs & retention guides..." color={COLORS.accent} />
          </View>
        )}
        ...
        {status === "error" && (
          <Card variant="error" style={styles.errorCard}>
            ...
            <Button
              label="Retry"
              onPress={() => rerunAgent("customer_service")}
              variant="danger"
              size="sm"
              style={{ marginTop: 10 }}
            />
        ...
        {!hasData && status === "idle" && (
          <View style={styles.empty}>
            <Headset size={48} color={COLORS.textMuted} style={{ marginBottom: 14, opacity: 0.4 }} />
            <Text style={styles.emptyTitle}>No Support Blueprint yet</Text>
            <Text style={styles.emptySub}>
              Run the startup plan generator to build customer onboarding and support workflows.
            </Text>
            <Button
              label="Generate Plan"
