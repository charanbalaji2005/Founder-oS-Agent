...
import { useFounder } from "@/hooks/useResearch";
...
import { ArrowLeft, RefreshCw, AlertTriangle, Crown, ChevronRight } from "lucide-react-native";

export default function FounderScreen() {
  const { content, status, error, hasData } = useFounder();
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <View style={styles.topBar}>
        ...
        <Text style={styles.topTitle}>Founder Agent</Text>
        {hasData && (
          <TouchableOpacity onPress={() => rerunAgent("founder")} style={styles.regenBtn}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.modelBadge}>
          <Text style={styles.modelBadgeText}>🧠 DeepSeek R1 via Groq</Text>
        </View>

        {status === "running" && (
          <View style={styles.loadingBox}>
            <Loader size={32} label="Structuring mission, vision & founder alignment plan..." color={COLORS.accent} />
          </View>
        )}
        ...
        {!hasData && status === "idle" && (
          <View style={styles.empty}>
            <Crown size={48} color={COLORS.textMuted} style={{ marginBottom: 14, opacity: 0.4 }} />
            <Text style={styles.emptyTitle}>No Founder Blueprint yet</Text>
            <Text style={styles.emptySub}>
              Run the startup generator pipeline to get co-founder alignment plans.
            </Text>
            <Button
              label="Generate Plan"
        ...
        {hasData && (
          <>
            ...
            <View style={styles.navRow}>
              <Button
                label="CEO Strategy"
                onPress={() => router.push("/startup/ceo")}
                variant="ghost"
                size="sm"
