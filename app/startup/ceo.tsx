...
import { useCEO } from "@/hooks/useResearch";
...
import { ArrowLeft, RefreshCw, AlertTriangle, Briefcase, ChevronRight } from "lucide-react-native";

export default function CEOScreen() {
  const { content, status, error, hasData } = useCEO();
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <View style={styles.topBar}>
        ...
        <Text style={styles.topTitle}>CEO Agent</Text>
        {hasData && (
          <TouchableOpacity onPress={() => rerunAgent("ceo")} style={styles.regenBtn}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {status === "running" && (
          <View style={styles.loadingBox}>
            <Loader size={32} label="Structuring monetization, targets & organizational charts..." color={COLORS.accent} />
          </View>
        )}
        ...
        {!hasData && status === "idle" && (
          <View style={styles.empty}>
            <Briefcase size={48} color={COLORS.textMuted} style={{ marginBottom: 14, opacity: 0.4 }} />
            <Text style={styles.emptyTitle}>No CEO Blueprint yet</Text>
            <Text style={styles.emptySub}>
              Run the startup generator pipeline to get organizational strategy plans.
            </Text>
            <Button
              label="Generate Plan"
          ...
          <>
            ...
            <View style={styles.navRow}>
              <Button
                ...
                onPress={() => router.push("/startup/research")}
                variant="ghost"
                size="sm"
