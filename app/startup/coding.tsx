...
import { useCoding } from "@/hooks/useResearch";
...
import { ArrowLeft, RefreshCw, AlertTriangle, Code, ChevronRight } from "lucide-react-native";

export default function CodingScreen() {
  const { content, status, error, hasData } = useCoding();
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <View style={styles.topBar}>
        ...
        <Text style={styles.topTitle}>Coding Agent</Text>
        {hasData && (
          <TouchableOpacity onPress={() => rerunAgent("coding")} style={styles.regenBtn}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.modelBadge}>
          <Text style={styles.modelBadgeText}>🧠 Qwen 2.5 Coder 32B via Groq</Text>
        </View>

        {status === "running" && (
          <View style={styles.loadingBox}>
            <Loader size={32} label="Generating technical architecture & database schema blueprints..." color={COLORS.accent} />
          </View>
        )}
        ...
        {!hasData && status === "idle" && (
          <View style={styles.empty}>
            <Code size={48} color={COLORS.textMuted} style={{ marginBottom: 14, opacity: 0.4 }} />
            <Text style={styles.emptyTitle}>No Tech Blueprint yet</Text>
            <Text style={styles.emptySub}>
              Run the startup generator pipeline to get database schemas and stack recommendations.
            </Text>
            <Button
              label="Generate Plan"
          ...
          <>
            ...
            <View style={styles.navRow}>
              <Button
                ...
                onPress={() => router.push("/startup/marketing")}
                variant="ghost"
                size="sm"
